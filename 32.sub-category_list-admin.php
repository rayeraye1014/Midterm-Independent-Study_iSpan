...
#算總筆數
$t_sql = "SELECT COUNT(1) FROM categories_sub";
...
if ($totalRows) {
  ...
  $sql = sprintf("SELECT * FROM categories_main join categories_sub on categories_sub.main_category = categories_main.id ORDER BY categories_sub.id DESC LIMIT %s, %s", ($page - 1) * $perPage, $perPage);
...
<script>
  ...
  document.getElementById('exportBtn').addEventListener('click', function() {
    if (confirm(`是否將子分類列表資料匯出csv檔?`)) {
      // 使用 fetch 進行 AJAX 請求
      fetch('39.file_csv-sub.php')
        .then(response => {
          if (!response.ok) {
            throw new Error(`HTTP error! Status: ${response.status}`);
          }
