...
$title = '商品子分類管理-耳機錄音';
$pageName = 'sub-category_earphone';
...
#算總筆數
$t_sql = "SELECT COUNT(1) FROM categories_main join categories_sub on categories_sub.main_category = categories_main.id WHERE main = '耳機錄音'";
...
if ($totalRows) {
    ...
    $sql = sprintf("SELECT * FROM categories_main join categories_sub on categories_sub.main_category = categories_main.id  WHERE main = '耳機錄音' ORDER BY categories_sub.id DESC LIMIT %s, %s", ($page - 1) * $perPage, $perPage);
