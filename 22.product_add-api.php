...
if (!empty($_POST)) {
    ...
    if (strlen($_POST['name']) < 1) {
        $output['error'] = '請填入商品名稱或超過字數限制(60)!';
        $output['code'] = 500;
        echo json_encode($output, JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    $sql = "INSERT INTO `products`
      (`seller_id`, `main_category`, `sub_category`, `product_photos`, `product_name`, `product_price`, `product_quantity`, `product_intro`, `carbon_points_available`, `created_at`, `edit_new`, `status_now`) 
      VALUES 
      (?, ?, ?, ?, ?, ?, ?, ? ,?, Now(), Now(), ?)";
