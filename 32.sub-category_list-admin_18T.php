...
#算總筆數
$t_sql = "SELECT COUNT(1) FROM categories_main join categories_sub on categories_sub.main_category = categories_main.id WHERE main = '門票票券'";
...
if ($totalRows) {
    ...
    $sql = sprintf("SELECT * FROM categories_main join categories_sub on categories_sub.main_category = categories_main.id  WHERE main = '門票票券' ORDER BY categories_sub.id DESC LIMIT %s, %s", ($page - 1) * $perPage, $perPage);
